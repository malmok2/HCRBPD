...
becomes a STRAIGHT line inclined by ALPHA.  So an unrolled patch is an in-line
...
to the plain in-line bundle, which is used as a regression check.
...
Run:  python helical_coil_mesh.py
"""
...
CLEARANCE = min(0.5 * ST - B_ELL, 0.5 * SL - A_ELL)    # tube -> cell boundary
...
#  neighbours, so the diagonal one never constrains the tube.
if CLEARANCE <= 0.0:
    fail("the tube footprint does not fit in its cell : the z-section is an "
         "ellipse %.3f x %.3f, cell is %.3f x %.3f"
         % (2 * A_ELL, 2 * B_ELL, SL, ST))
...
print("        faceted tubes (%d-gon) hold %.3f %% more fluid than true ellipses"
...
print(" around tube %d | radial %d (first %.4f, growth %.3f) | flow A %d / C %d"
      % (4 * N_AZ, N_RAD, CLEARANCE * FR_RAD[1], Q_RAD, N_X_IN, N_X_OUT))
