...
# -*- coding: utf-8 -*-
"""
In-line (lattice) rod bundle : structured all-hexahedral mesh generator
=======================================================================
Standalone.  No SALOME, no CAD kernel, no third-party module - stdlib only.

Sister tool to the staggered generator; only the lattice and the cell shape
differ, everything downstream is identical.
...
algorithm is involved:
  ...
  B : rods sit on a rectangular lattice, so the Voronoi cell of each rod is
      simply the SL x ST rectangle around it.  Radial lines from the rod to the
      four corners cut it into one mapped block per side (an O-grid).  Radial
      spacing is geometric -> wall inflation.
...
Compared with the staggered version the rectangular cell needs no squaring-off
at the bundle ends (its left and right sides already lie on the interface
planes), which removes a special case.  In exchange every cell corner is a 90
deg angle split by one radial line, so corner cells sit near 45 deg skew.
...
Run:  python rod_lattice_mesh.py
"""
...
HALF_RODS_AT_WALL = True   # True : rods on y = 0 and y = W, halved by the walls
                           #        (periodic lattice, N_COLS+1 rods per row)
                           # False: rods at mid-pitch, walls stay clear
                           #        (N_COLS rods per row)

# --- mesh -------------------------------------------------------------------
N_AZ_MIN    = 36       # minimum azimuthal cells around one rod (rounded up to 8*n)
N_RAD       = 12       # radial cells, rod wall -> cell boundary
FIRST_LAYER = 0.15     # first cell height on the rod wall   [mm]

N_Z         = 10       # layers in z  (1 = single layer, quasi 2-D)
...
HL        = 0.5 * SL                                   # cell half length
S_D       = math.hypot(SL, ST)                         # diagonal neighbour distance
CLEARANCE = min(0.5 * ST, 0.5 * SL) - R                # rod wall -> cell boundary
N_AZ      = 2 * max(1, int(math.ceil(N_AZ_MIN / 8.0)))  # per full side; even so a
                                                        # halved side splits exactly
SCALE     = max(L_TOT, W, H)
TOL       = 1.0e-9 * SCALE
...
#  On a rectangular lattice the cell is bounded only by the four nearest
#  neighbours, so the diagonal one never constrains the rod.
if CLEARANCE <= 0.0:
    fail("the rod does not fit in its cell : need D < min(ST, SL) = %.3f"
         % (2.0 * min(0.5 * ST, 0.5 * SL)))
...
# =============================================================================
def lattice():
    """In-line arrangement: every row carries rods at the same y positions."""
    cells = []
    for i in range(N_ROWS):
        x = X_B0 + 0.5 * SL + i * SL
        if HALF_RODS_AT_WALL:
            for j in range(N_COLS + 1):       # y = 0 .. W, the ends halved by the walls
                cells.append((x, j * ST, True, i))
        else:
...
def cell_polygon(cx, cy, i_row):
    """CCW Voronoi cell of an in-line lattice: the SL x ST rectangle.

    No squaring-off is needed at the bundle ends - the rectangle's left and
    right sides already sit exactly on the A|B and B|C planes."""
    pts = [(cx + HL, cy - HH), (cx + HL, cy + HH),
           (cx - HL, cy + HH), (cx - HL, cy - HH)]
...
def side_div(ia, ib):
    ...
    A side cut in half by a channel wall gets half the divisions, so the cells
    there keep the same size as everywhere else.  Both cells sharing a side
    compute the same value, so the mesh stays conformal."""
    a, b = VERT[ia], VERT[ib]
    dx, dy = abs(b[0] - a[0]), abs(b[1] - a[1])
    full = SL if dx > dy else ST                    # a full side of that orientation
    return max(1, int(round(N_AZ * math.hypot(dx, dy) / full)))
...
QZONE = []          # 0 = A, 1 = B, 2 = C
_maxdev = [0.0]
ROD_AREA = [0.0]    # area actually removed by the faceted rods
        ...
        for k in range(n):
            ...
            def pos(i, j, ia=ia, ib=ib, tA=tA, tB=tB, cx=cx, cy=cy, ns=ns):
                if i == 0:
                    ...
                    S = (cx + R * math.cos(th), cy + R * math.sin(th))
            ...
            ROD_AREA[0] += 0.5 * R * R * ns * math.sin((tB - tA) / ns)
...
def on_boundary_2d(a, b):
    ...
    if abs(A[1] - W) < TOL and abs(B[1] - W) < TOL:
        return True
    for (cx, cy) in _CEN:
        if (abs(math.hypot(A[0] - cx, A[1] - cy) - R) < 1e-6 * SCALE and
                abs(math.hypot(B[0] - cx, B[1] - cy) - R) < 1e-6 * SCALE):
            return True
    return False
...
POINTS = [(x, y, z) for z in ZS for (x, y) in NODES]
...
for e in PATCHES["wall_rods"]:
    if not any(all(abs(math.hypot(POINTS[i][0] - cx, POINTS[i][1] - cy) - R) < 1e-6 * SCALE
                   for i in e[0]) for (cx, cy) in CENTERS):
        bad += 1
if bad:
...
circle = (L_TOT * W - sum(math.pi * R * R * (0.5 if (cy < TOL or cy > W - TOL) else 1.0)
...
print("        faceted rods (%d-gon) hold %.3f %% more fluid than exact circles"
      % (4 * N_AZ, 100.0 * (vtot - circle) / circle))
print("[check] skew    max %.1f deg   <15 %.1f%% | 15-30 %.1f%% | 30-40 %.1f%% | "
      "40-50 %.1f%% | >50 %.1f%%"
      % ((dev_max,) + tuple(100.0 * b / len(QUAL) for b in bins)))
print("        %d cells sit on a cell corner (~%.0f deg); every rectangle corner is"
      % (sum(1 for q in QUAL if abs(q[0] - dev_max) < 0.05), dev_max))
print("        a 90 deg angle split by one radial line, so this is the design floor")
...
def write_fluent(path):
    with open(path, "w") as f:
        f.write('(0 "rod bundle : structured hexahedral mesh")\n')
        f.write("(0 \"dimension\")\n(2 3)\n\n")
        f.write('(0 "nodes")\n(10 (0 1 %x 0 3))\n' % len(POINTS))
...
print("-" * 68)
print(" lattice    : in-line, cell %.3f x %.3f (SL x ST), clearance %.4f"
      % (SL, ST, CLEARANCE))
print(" domain %.2f x %.2f x %.2f | rods %d | cells %d"
      % (L_TOT, W, H, N_RODS, len(HEXES)))
print(" azimuthal %d/rod | radial %d (first %.4f, growth %.3f) | z %d"
      % (4 * N_AZ, N_RAD, CLEARANCE * FR_RAD[1], Q_RAD, N_Z))
print("-" * 68)
