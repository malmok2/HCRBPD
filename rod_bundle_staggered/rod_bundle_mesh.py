...
# -*- coding: utf-8 -*-
"""
Staggered rod bundle : structured all-hexahedral mesh generator
================================================================
...
algorithm is involved:
  ...
  B : each rod sits in its Voronoi hexagon of the staggered lattice; radial
      lines from the rod to the hexagon corners cut it into one mapped block
...
SL          = 17.32    # longitudinal pitch (x)              [mm]  (ST*sqrt(3)/2 -> equilateral)
...
HALF_RODS_AT_WALL = True   # True : aligned rows carry half rods on the side walls
FIRST_ROW_OFFSET  = True   # True : row 0 is the offset (centred) row

# --- mesh -------------------------------------------------------------------
N_AZ_MIN    = 36       # minimum azimuthal cells around one rod (rounded up to 6*n)
N_RAD       = 12       # radial cells, rod wall -> hexagon boundary
...
X_B1      = L_IN + L_BUND
HH        = 0.5 * ST
XV        = 0.5 * SL + ST * ST / (8.0 * SL)
XE        = 0.5 * SL - ST * ST / (8.0 * SL)
S_D       = math.hypot(SL, 0.5 * ST)
CLEARANCE = min(0.5 * ST, 0.5 * S_D, 0.5 * SL) - R
N_AZ      = max(1, int(math.ceil(N_AZ_MIN / 6.0)))     # per hexagon side
SCALE     = max(L_TOT, W, H)
TOL       = 1.0e-9 * SCALE
...
if SL <= 0.5 * ST:
    fail("SL <= ST/2 : the staggered Voronoi cell degenerates")
if CLEARANCE <= 0.0:
    fail("the rod does not fit in its cell : need D < min(ST, S_D, SL) = %.3f"
         % (2.0 * min(0.5 * ST, 0.5 * S_D, 0.5 * SL)))
...
# --- B : one O-grid block per hexagon side ----------------------------------
def build_bundle():
    for (cx, cy, rod, vids) in PCELL:
        ...
        for k in range(n):
            ...
            def pos(i, j, ia=ia, ib=ib, tA=tA, tB=tB, cx=cx, cy=cy):
                if i == 0:
                    S = wall_pt(ia)
                elif i == N_AZ:
                    S = wall_pt(ib)
                else:
                    th = tA + (tB - tA) * (i / float(N_AZ))
                    S = (cx + R * math.cos(th), cy + R * math.sin(th))
                return lerp(S, side_point(ia, ib, i, N_AZ), FR_RAD[j])
            ...
            ROD_AREA[0] += 0.5 * R * R * N_AZ * math.sin((tB - tA) / N_AZ)

            def key(i, j, ia=ia, ib=ib, cid=cid, k=k):
                if j == N_RAD:                      # on the cell side
                    if i == 0:                      # corner: shared by 3 sides
                        return ("vert", ia)
                    if i == N_AZ:
                        return ("vert", ib)
                    return side_key(ia, ib, i, N_AZ)
                if i == 0:                          # radial ray, shared with sector k-1
                    return ("ray", cid, ia, j)
                if i == N_AZ:                       # radial ray, shared with sector k+1
                    return ("ray", cid, ib, j)
                if j == 0:
                    return ("arc", cid, k, i)
                return ("o", cid, k, i, j)

            add_block(N_AZ, N_RAD, pos, key, 1)
...
def build_box(x_plane, length, nx, fr, zone, tag):
    ...
    for (va, vb) in interface_sides(x_plane):
        ...
        def pos(i, k, va=va, vb=vb):
            P = side_point(va, vb, i, N_AZ)
            return (x_plane + sgn * length * fr[k], P[1])

        def key(i, k, va=va, vb=vb):
            if k == 0:                              # on the A|B (B|C) interface
                if i == 0:
                    return ("vert", va)
                if i == N_AZ:
                    return ("vert", vb)
                return side_key(va, vb, i, N_AZ)
        ...
        add_block(N_AZ, nx, pos, key, zone)
...
print("        faceted rods (%d-gon) hold %.3f %% more fluid than exact circles"
      % (6 * N_AZ, 100.0 * (vtot - circle) / circle))
print("[check] skew    max %.1f deg   <15 %.1f%% | 15-30 %.1f%% | 30-40 %.1f%% | "
      "40-50 %.1f%% | >50 %.1f%%"
      % ((dev_max,) + tuple(100.0 * b / len(QUAL) for b in bins)))
print("        %d cells sit on the hexagon corner angle (~30 deg); the worst cells"
      % sum(1 for q in QUAL if abs(q[0] - 30.0) < 0.05))
print("        are the four corners of the squared-off first/last row cells")
...
print(" azimuthal %d/rod | radial %d (first %.4f, growth %.3f) | z %d"
      % (6 * N_AZ, N_RAD, CLEARANCE * FR_RAD[1], Q_RAD, N_Z))
print("-" * 68)
